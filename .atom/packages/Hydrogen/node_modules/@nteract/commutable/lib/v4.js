...
function createImmutableAttachments(attachments) {
    if (!attachments) {
        return undefined;
    }
    return immutable_1.Map(attachments).map((bundle) => {
        for (const mime_type in bundle) {
            if (bundle.hasOwnProperty(mime_type)) {
                bundle[mime_type] = primitives_1.demultiline(bundle[mime_type]);
            }
        }
        return bundle;
    });
...
function attachmentsToJS(immAttachments) {
    return immAttachments === null || immAttachments === void 0 ? void 0 : immAttachments.toJS();
}
